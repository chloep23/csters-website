...
export default function AlumniDirectory() {
  ...
  return (
    <div style={{ backgroundColor: "#f0ece3" }} className="min-h-screen pt-20 pb-16 px-6 sm:px-12">
      <h1 className="text-2xl sm:text-3xl font-bold tracking-widest uppercase text-black text-center mb-8">
        Alumni Directory
      </h1>
      ...
      {loading ? (
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          {alumni.map((person, i) => (
            <div
              key={i}
              className="text-center px-3 sm:px-6 py-4 sm:py-8 border-b border-gray-200"
            >
              <p className="text-black font-medium text-sm sm:text-base mb-1">{person.name}</p>
              <p className="text-black text-xs sm:text-sm">{person.email}</p>
